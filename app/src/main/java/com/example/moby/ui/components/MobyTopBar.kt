...
fun MobyTopBar(
    ...
    val containerBg = if (isAbisal) {
        Color(0xCC121324) // 80% Abisal Surface
    } else {
        Color(0xCCFFFFFF) // 80% White
    }
    
    val borderStrokeColor = if (isAbisal) {
        Color(0x1AFFFFFF) // Light border for Abisal mode
    } else {
        Color(0x1A000000) // Soft border for Arrecife mode
    }
